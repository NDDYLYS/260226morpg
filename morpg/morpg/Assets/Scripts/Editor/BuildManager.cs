...
public class BuildManager
{
    ...
    [MenuItem("BuildManager/Build/PC(64bit)")]

    private static void Build_PC_64bit()
    {
        var startTime = DateTime.Now;
        UnityEngine.Debug.Log(string.Format("<color=cyan>PC Build Start.</color> <color=yellow>StartTime : {0}</color>", startTime));

        var path = Path.GetFullPath("F://Build/morpg");
        ...
        var day = string.Format("{0:yy.MM.dd}", DateTime.Now);
        path = string.Format("{0}/{1}", path, day);
        if (!File.Exists(path))
            Directory.CreateDirectory(path);

        var hour = string.Format("{0:HHmmss}", DateTime.Now);
        path = string.Format("{0}/{1}", path, hour);
        if (!File.Exists(path))
            Directory.CreateDirectory(path);

        var exe = string.Format("morpg.exe");
        var exePath = string.Format("{0}/{1}", path, exe);
        ...
        if (summary.result == BuildResult.Succeeded)
        {
            ...
            UnityEngine.Debug.Log(string.Format("<color=cyan>PC(64bit) Build Success => {0}</color> <color=yellow>EndTime : {1}({2})</color>", GetFilesSize(summary.totalSize), endTime, gap.TotalMilliseconds));
        }
        else
        {
            var endTime = DateTime.Now;
            ...
            UnityEngine.Debug.Log(string.Format("<color=red>PC(64bit) Build Fail.</color> <color=yellow>EndTime : {0}({1})</color>\n<color=blue>Error.Count : {2}</color> <color=red>Warning.Count : {3}</color>", endTime, gap.TotalMilliseconds, res.summary.totalErrors, res.summary.totalWarnings));
    ...
    private static string GetFilesSize(ulong _byte)
    {
        var size = "0 bytes";

        if (MathF.Pow(2, 30) <= _byte)
            size = string.Format("{0:##.##} GB", _byte / MathF.Pow(2, 30));
        else if (MathF.Pow(2, 20) <= _byte)
            size = string.Format("{0:##.##} MB", _byte / MathF.Pow(2, 20));
        else if (MathF.Pow(2, 10) <= _byte)
            size = string.Format("{0:##.##} KB", _byte / MathF.Pow(2, 10));
        else if (0 <= _byte)
            size = string.Format("{0:##.##} bytes", _byte);
        return size;
    }
